...
class Tokenizer_Base
{
    ...
    {
        $token_start = (($this->token_start !== null) ? $this->token_start : $this->pos) + $start_offset;
     ...
     */
    protected function parse_linebreak($cur_char)
    {
        if ($cur_char === "\n") {
            ++$this->line_pos[0];
    ...
    public function next_no_whitespace()
    {
        ...
        while (++$this->pos < $this->size) {
            ...
            if (isset($this->whitespace[$char])) {
                $this->parse_linebreak($char);
            } elseif (isset($this->char_map[$char])) {
                $token = $this->char_map[$char];
                return ($this->token = is_string($token) ? $this->{$token}() : $token);
            } else {
                return ($this->token = self::TOK_UNKNOWN);
    ...
    {
        $start_pos = $this->pos + 1;
        $len = $pos - $start_pos;
        if ($len > 0) {
            $str = substr($this->doc, $start_pos, $len);

            if (($l = strrpos($str, "\n")) !== false) {
                ++$this->line_pos[0];
                $this->line_pos[1] = $l + $start_pos;
