...
class HTML_Node
{
     ...
     * @see select()
     */
    public $selectClass = 'HTML_Selector';
    /**
     ...
     * @see setInnerText()
     */
    public $parserClass = 'HTML_Parser_HTML5';
     ...
     * @var string
     */
    public $childClass_Text = 'HTML_Node_Text';
    /**
     * Name of the class used for {@link addComment()}
     * @var string
     */
    public $childClass_Comment = 'HTML_Node_Comment';
    /**
     * Name of the class used for {@link addContional()}
     * @var string
     */
    public $childClass_Conditional = 'HTML_Node_Conditional';
    /**
     * Name of the class used for {@link addCDATA()}
     * @var string
     */
    public $childClass_CDATA = 'HTML_Node_Cdata';
    /**
     * Name of the class used for {@link addDoctype()}
     * @var string
     */
    public $childClass_Doctype = 'HTML_Node_Doctype';
    /**
     * Name of the class used for {@link addXML()}
     * @var string
     */
    public $childClass_XML = 'HTML_Node_XML';
    /**
     * Name of the class used for {@link addASP()}
     * @var string
     */
    public $childClass_ASP = 'HTML_Node_ASP';
    ...
    protected static $short_attributes = array(
        ...
        'script' => array('async' => true, 'defer' => true, 'nomodule' => true),
    ...
    public function __construct($tag, $parent)
    {
        ...
        if (is_string($tag)) {
            ...
            $this->class = isset($this->attributes['class']) ? $this->attributes['class'] : null;
        }
    }
    ...
    protected function toString_attributes()
    {
        ...
        foreach ($this->attributes as $a => $v) {
            $s .= ' ' . $a;
            if ($v !== false && !($this->attribute_shorttag && ($v === $a) && isset(self::$short_attributes[$this->tag][$a]))
            ) {
    ...
    public function getNamespace()
    {
        if ($this->tag_ns === null) {
            ...
            $this->tag_ns = empty($a[1]) ? array('', $a[0]) : array($a[0], $a[1]);
        }
     ...
     * @return string|false False if encoding could not be found
     */
     ...
     * @param bool $hidden True for <!--[if, false for <![if
     * @param int $offset Position to insert node, negative to count from end, null to append
     * @return HTML_Node Added node
     * @see addChild();
     */
     ...
     * @return array|false array('ns', 'attr', 'ns:attr', index)
     * @access private
     */
     ...
     * @param bool $case_sensitive Compare with case sensitivity
     */
    public function removeAttribute($attr, $compare = 'total', $case_sensitive = false)
    {
        $this->deleteAttribute($attr, $compare, $case_sensitive);
    }
    ...
    public function addClass($className)
    {
        ...
        foreach ($className as $c) {
            if ($class === null) {
                $class = $c;
            } elseif (!(preg_match('/(?:^|\s)' . preg_quote($c, '/') . '(?:$|\s)/si', $class) > 0)) {
                $class .= ' ' . $c;
            }
        }
    ...
    public function removeClass($className)
    {
        ...
        $class = $this->class;
        foreach ($className as $c) {
            $class = ltrim(preg_replace('/(^|\s)' . preg_quote($c, '/') . '(?:$|\s+)/si', '\1', $class));
        }
        if ($class) {
            $this->class = $this->attributes['class'] = $class;
        } else {
            unset($this->class, $this->attributes['class']);
        }
    }
    ...
    public function getChildrenByCallback($callback, $recursive = true, $check_self = false)
    {
        ...
        $res = ($check_self && $callback($this)) ? array($this) : array();
    ...
    {
        $count = $this->childCount();
        $res = ($check_self && $this->match($conditions, true, $custom_filters)) ? array($this) : array();
    ...
    protected function match_attributes($attributes)
    {
        ...
        foreach ($attributes as $attribute => $match) {
            ...
            if (!($res && $match['operator_result'] === 'or')) {
                ...
                if (!$res && $has && is_string($match['value'])) {
                    foreach ($possibles as $a) {
                        ...
                        switch ($match['operator_value']) {
                            ...
                            case '^=':
                            case 'starts_with':
                                $res = ((strncmp($val, $match['value'], strlen($match['value'])) === 0) === $match['match']);
                                if ($res) break 1; else break 2;
