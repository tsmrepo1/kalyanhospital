...
class HTML_Selector
{
     ...
     */
    public $parser = 'Tokenizer_CSSQuery';
     ...
     * @param string $query
     * @return HTML_Node|HTML_Node[]|false
     * @access private
     */
     ...
     * Perform query
     * @param string $query
     * @return HTML_Node|HTML_Node[]|false False on failure
     */
     ...
     * Trigger error
     * @param string $error
     * @internal %pos% and %tok% will be replaced in string with position and token(string)
     * @access private
     */
    protected function error($error)
    {
        $error = htmlentities(str_replace(
            array('%tok%', '%pos%'),
            array($this->parser->getTokenString(), $this->parser->getPos()),
            $error
        ));
        ...
        while (count($this->result) > 0) {
            switch ($p->token) {
                ...
                case Tokenizer_CSSQuery::TOK_COLON:
                    ...
                    if (!$this->parse_result(!$whitespace_skipped)) {
                        return false;
                    }
                    break;
